...
logger = get_logger("file-writer-minio")
...
def create_minio_client(minio_config: MinioProperties | None = None) -> Minio:
    ...
    Args:
        minio_config (MinioProperties | None): Configuration class for Minio client.
...
class MinioFileWriter(FileWriterBase):
    ...
    def write_file(self, messages: list[str], unique_consumer_id: str) -> None:
        ...
        logger.debug(f"(6) Saving. JSON file saved to MinIO at: {file_path}")
