...
async def store_event(
    ...
    if event_item.event_name in events_mapping:
        ...
        # (6) Trigger the on_delivery report callback
        # NOTE: This method polls the queue generated by producer for events and calls
        # the corresponding callbacks (if registered)
        # Timeout is set to 0, not to spend too much time if the queue is empty
        # Note that due to the asyc nature of the queue formation there migh be no message polled
        # from the queue (since it is not yet there or is already flushed), so there is no
        # guarantee that on_delivery report callback will be absolutely triggered for every message
